...
class ICE40Platform(TemplatedPlatform):
	'''
		...
		**TemplatedPlatform.build_script_templates,
		...
		'{{name}}.pcf': r'''
			...
			{% for port_name, pin_name, attrs in platform.iter_port_constraints_bits() -%}
				{%- if 'PULLUP' in attrs and attrs['PULLUP'] == 1 -%}
					{%- if 'RESISTOR' in attrs %}
						{% set pullup %}-pullup yes -pullup_resistor {{ attrs['RESISTOR'] }} {% endset %}
					{%- else -%}
						{% set pullup = '-pullup yes ' %}
					{%- endif -%}
				{%- else -%}
					{% set pullup = "" %}
				{%- endif -%}
				set_io {{ pullup }}{{port_name}} {{pin_name}}
			{% endfor %}
			{% for net_signal, port_signal, frequency in platform.iter_clock_constraints() -%}
				set_frequency {{net_signal|hierarchy(".")}} {{ '{:#MHz}'.format(frequency) }}
			{% endfor%}
		...
		if name == 'sync' and self.default_clk is not None:
			...
			if self.default_clk == 'SB_HFOSC':
				...
				if not isinstance(self.hfosc_div, int) or self.hfosc_div < 0 or self.hfosc_div > 3:
					raise ValueError(
						'SB_HFOSC divider exponent (hfosc_div) must be an integer between 0 and 3, '
						f'not {self.hfosc_div!r}'
					)
				clk_i = Signal()
				...
				delay = int(100e-6 * self.default_clk_frequency.hertz)
			...
			# User-defined clock signal.
				...
				delay = int(15e-6 * self.default_clk_frequency.hertz)
