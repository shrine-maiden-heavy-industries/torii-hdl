...
class MachXO2Or3LPlatform(TemplatedPlatform):
	...
	Available overrides:
		...
		* ``synth_opts``: adds options for ``synth_lattices`` Yosys command.
		...
		* ``nextpnr_opts``: adds extra options for ``nextpnr-machxo2``.
	...
	_trellis_file_templates = {
		**TemplatedPlatform.build_script_templates,
		...
		'{{name}}.ys': r'''
			...
			synth_lattice -family {{'xo2' if platform.device.startswith('LCMXO3L') else 'xo3'}} {{get_override("synth_opts")|options}} -top {{name}}
		...
		'{{name}}.lpf': r'''
			...
			{% for net_signal, port_signal, frequency in platform.iter_clock_constraints() -%}
				{% if port_signal is not none and net_signal not in platform._active_extrefs -%}
					FREQUENCY PORT "{{port_signal.name}}" {{ '{:#Hz}'.format(frequency) }} HZ;
				{% else -%}
					FREQUENCY NET "{{net_signal|hierarchy(".")}}" {{ '{:#Hz}'.format(frequency) }} HZ;
				{% endif %}
			{% endfor %}
	...
	def __init__(self, *, toolchain: Literal['Trellis', 'Diamond'] = 'Trellis') -> None:
		super().__init__()
