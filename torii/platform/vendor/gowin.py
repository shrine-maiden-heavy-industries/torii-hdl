...
class GowinPlatform(TemplatedPlatform):
	...
	def _osc_type(self):
		...
		raise NotImplementedError(f"Device family {self.family} does not have an assigned oscillator type")
	...
	def _osc_base_freq(self):
		...
		if osc == "OSC":
			...
			raise ValueError(
				f"Unknown oscillator, expected 'OSC', 'OSCZ', 'OSCO', 'OSCF', 'OSCH', 'OSCW', or 'OSCA' not '{osc}'"
			)
	...
	def _osc_div(self):
		...
		if (abs(round(div_frac) - div_frac) > Fraction(50, 1_000_000) or (int(div_frac) not in div_range)) and (
			not divides_by_3 if support_div_by_3 else True
		):
			achievable = [
		...
		if name == "sync" and self.default_clk is not None:
			...
			if self.default_clk == "OSC":
				...
				if self._osc_type == "OSCA":
					...
					m.submodules += Instance(self._osc_type, p_FREQ_DIV=self._osc_div, o_OSCOUT=clk_i)
	...
	def _get_xdr_buffer(self, m, pin, i_invert=False, o_invert=False):
		...
		def get_oreg(clk, d, q):
			for bit in range(len(q)):
				m.submodules += Instance("DFF", i_CLK=clk, i_D=d[bit], o_Q=q[bit])

		def get_iddr(clk, d, q0, q1):
			for bit in range(len(d)):
				m.submodules += Instance("IDDR", i_CLK=clk, i_D=d[bit], o_Q0=q0[bit], o_Q1=q1[bit])
	...
	def get_input(self, pin: Pin, port: Record, attrs: Attrs, invert: bool, names: Iterable[str]) -> Module:
		self._check_feature(
			PinFeature.SE_INOUT,
			pin,
			attrs,
		...
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance("IBUF", i_I=port.io[bit], o_O=i[bit])
		return m

	def get_output(self, pin: Pin, port: Record, attrs: Attrs, invert: bool, names: Iterable[str]) -> Module:
		self._check_feature(
			PinFeature.SE_OUTPUT,
			pin,
			attrs,
			valid_xdrs=(0, 1, 2),
			valid_attrs=True,
		...
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance("OBUF", i_I=o[bit], o_O=port.io[bit])
		return m

	def get_tristate(self, pin: Pin, port: Record, attrs: Attrs, invert: bool, names: Iterable[str]) -> Module:
		self._check_feature(
			PinFeature.SE_TRISTATE,
			pin,
			attrs,
			valid_xdrs=(0, 1, 2),
			valid_attrs=True,
		...
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance("TBUF", i_OEN=t, i_I=o[bit], o_O=port.io[bit])
		return m

	def get_input_output(self, pin: Pin, port: Record, attrs: Attrs, invert: bool, names: Iterable[str]) -> Module:
		self._check_feature(
			PinFeature.SE_INOUT,
			pin,
			attrs,
		...
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance("IOBUF", i_OEN=t, i_I=o[bit], o_O=i[bit], io_IO=port.io[bit])
		return m
	...
	def get_diff_input(
		...
		attrs: Attrs,
		invert: bool,
		names: tuple[Iterable[str], Iterable[str]],
	) -> Module:
		self._check_feature(
			PinFeature.DIFF_INPUT,
			pin,
			attrs,
			valid_xdrs=(0, 1, 2),
			valid_attrs=True,
		...
		i, o, t = self._get_xdr_buffer(m, pin, i_invert=invert)
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance("TLVDS_IBUF", i_I=port.p[bit], i_IB=port.n[bit], o_O=i[bit])
		return m
	...
	def get_diff_tristate(
		...
		for bit in range(pin.width):
			m.submodules[f"{pin.name}_{bit}"] = Instance(
				"TLVDS_TBUF", i_OEN=t, i_I=o[bit], o_O=port.p[bit], o_OB=port.n[bit]
			)
		return m
