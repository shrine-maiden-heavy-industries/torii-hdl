		...
		'{{name}}.xdc': r'''
			...
			{% for net_signal, port_signal, frequency in platform.iter_clock_constraints() -%}
				{% if port_signal is not none -%}
					create_clock -name {{port_signal.name|ascii_escape}} -period {{frequency.period.nanoseconds}} [get_ports {{port_signal.name|tcl_escape}}]
				{% else -%}
					create_clock -name {{net_signal.name|ascii_escape}} -period {{frequency.period.nanoseconds}} [get_nets {{net_signal|hierarchy("/")|tcl_escape}}]
				{% endif %}
			{% endfor %}
		...
		**TemplatedPlatform.build_script_templates,
		...
		'{{name}}.ucf': r'''
			...
			{% for net_signal, port_signal, frequency in platform.iter_clock_constraints() -%}
				...
				TIMESPEC "TS{{net_signal|hierarchy("__")}}" = PERIOD "PRD{{net_signal|hierarchy("/")}}" {{frequency.period.nanoseconds}} ns HIGH 50%;
			{% endfor %}
			{{get_override("add_constraints")|default("# (add_constraints placeholder)")}}
		''' # noqa: E501
	}
		...
		'{{name}}.sdc': r'''
			...
			{% for net_signal, port_signal, frequency in platform.iter_clock_constraints() -%}
				{% if port_signal is none -%}
					create_clock -period {{frequency.period.nanoseconds}} {{net_signal.name|ascii_escape}}
				{% endif %}
			{% endfor %}
		'''
	}
