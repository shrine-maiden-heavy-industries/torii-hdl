...
def PCIeBusResources(
	...
	wake_n: str | None = None, sda: str | None = None, scl: str | None = None, clkreq_n: str | None = None,
	pwrbrk_n: str | None = None, tck: str | None = None, tdi: str | None = None, tdo: str | None = None,
	tms: str | None = None, trst_n: str | None = None, invert: bool = False,
	conn: ResourceConn | None = None, attrs: Attrs | None = None
) -> list[Resource]:
	'''
	...
	* x32
	...
	io_common.append(Subsignal('perst', PinsN(perst_n, dir = 'i', conn = conn, assert_width = 1)))
	io_common.append(Subsignal('refclk', DiffPairs(refclk_p, refclk_n, dir = 'i', conn = conn, assert_width = 1)))

	jtag_sigs = (tck, tdi, tdo, tms, trst_n,) # type: ignore

	if wake_n is not None:
		io_common.append(Subsignal('wake', PinsN(wake_n, dir = 'i', conn = conn, assert_width = 1)))

	if sda is not None and scl is not None:
		io_common.append(Subsignal('sda', Pins(sda, dir = 'io', conn = conn, assert_width = 1)))
		io_common.append(Subsignal('scl', Pins(scl, dir = 'io', conn = conn, assert_width = 1)))
	...
	if all(jtag_sigs):
		...
		io_common.append(Subsignal('tdo', Pins(jtag_sigs[2], dir = 'o', conn = conn, assert_width = 1)))
		io_common.append(Subsignal('tms', Pins(jtag_sigs[3], dir = 'i', conn = conn, assert_width = 1)))
		io_common.append(Subsignal('trst', PinsN(jtag_sigs[4], dir = 'i', conn = conn, assert_width = 1)))

	if clkreq_n is not None:
		io_common.append(Subsignal('clkreq', PinsN(clkreq_n, dir = 'o', conn = conn, assert_width = 1)))

	if attrs is not None:
		io_common.append(attrs)

	io_x1 = list(io_common)
	io_x1.append(Subsignal('pet0', DiffPairs(pet0_p, pet0_n, dir = 'o', conn = conn, assert_width = 1)))
	io_x1.append(Subsignal('per0', DiffPairs(per0_p, per0_n, dir = 'i', conn = conn, assert_width = 1)))

	resources.append(Resource.family(
		name_or_number, number, default_name = 'pcie', ios = io_x1, name_suffix = 'x1'
	))

	io_x2: list[SubsigArgT] | None = None
	io_x4: list[SubsigArgT] | None = None
	io_x6: list[SubsigArgT] | None = None # XXX(aki): Non-standard
	io_x8: list[SubsigArgT] | None = None
	io_x12: list[SubsigArgT] | None = None # XXX(aki): PCIe <= 4.0
	io_x16: list[SubsigArgT] | None = None
	io_x24: list[SubsigArgT] | None = None # XXX(aki): Non-standard
	io_x32: list[SubsigArgT] | None = None # XXX(aki): PCIe <= 4.0
	...
	if all(x2_sigs):
		...
		io_x2.append(Subsignal('pet1', DiffPairs(x2_sigs[0], x2_sigs[1], dir = 'o', conn = conn, assert_width = 1)))
		io_x2.append(Subsignal('per1', DiffPairs(x2_sigs[2], x2_sigs[3], dir = 'i', conn = conn, assert_width = 1)))
	...
	if io_x2 is not None:
		if all(x4_sigs):
			...
			io_x4.append(Subsignal('pet2', DiffPairs(x4_sigs[0], x4_sigs[1], dir = 'o', conn = conn, assert_width = 1)))
			io_x4.append(Subsignal('per2', DiffPairs(x4_sigs[2], x4_sigs[3], dir = 'i', conn = conn, assert_width = 1)))
			io_x4.append(Subsignal('pet3', DiffPairs(x4_sigs[4], x4_sigs[5], dir = 'o', conn = conn, assert_width = 1)))
			io_x4.append(Subsignal('per3', DiffPairs(x4_sigs[6], x4_sigs[7], dir = 'i', conn = conn, assert_width = 1)))
	...
	if io_x4 is not None:
		if all(x6_sigs):
			...
			io_x6.append(Subsignal('pet4', DiffPairs(x6_sigs[0], x6_sigs[1], dir = 'o', conn = conn, assert_width = 1)))
			io_x6.append(Subsignal('per4', DiffPairs(x6_sigs[2], x6_sigs[3], dir = 'i', conn = conn, assert_width = 1)))
			io_x6.append(Subsignal('pet5', DiffPairs(x6_sigs[4], x6_sigs[5], dir = 'o', conn = conn, assert_width = 1)))
			io_x6.append(Subsignal('per5', DiffPairs(x6_sigs[6], x6_sigs[7], dir = 'i', conn = conn, assert_width = 1)))
	...
	if io_x6 is not None:
		if all(x8_sigs):
			...
			io_x8.append(Subsignal('pet6', DiffPairs(x8_sigs[0], x8_sigs[1], dir = 'o', conn = conn, assert_width = 1)))
			io_x8.append(Subsignal('per6', DiffPairs(x8_sigs[2], x8_sigs[3], dir = 'i', conn = conn, assert_width = 1)))
			io_x8.append(Subsignal('pet7', DiffPairs(x8_sigs[4], x8_sigs[5], dir = 'o', conn = conn, assert_width = 1)))
			io_x8.append(Subsignal('per7', DiffPairs(x8_sigs[6], x8_sigs[7], dir = 'i', conn = conn, assert_width = 1)))
	...
	if io_x8 is not None:
		if all(x12_sigs):
			...
			io_x12.append(
				Subsignal('pet8', DiffPairs(x12_sigs[0], x12_sigs[1], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('per8', DiffPairs(x12_sigs[2], x12_sigs[3], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('pet9', DiffPairs(x12_sigs[4], x12_sigs[5], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('per9', DiffPairs(x12_sigs[6], x12_sigs[7], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('pet10', DiffPairs(x12_sigs[8], x12_sigs[9], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('per10', DiffPairs(x12_sigs[10], x12_sigs[11], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('pet11', DiffPairs(x12_sigs[12], x12_sigs[13], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x12.append(
				Subsignal('per11', DiffPairs(x12_sigs[14], x12_sigs[15], dir = 'i', conn = conn, assert_width = 1))
			)
	...
	if io_x12 is not None:
		if all(x16_sigs):
			...
			io_x16.append(
				Subsignal('pet12', DiffPairs(x16_sigs[0], x16_sigs[1], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('per12', DiffPairs(x16_sigs[2], x16_sigs[3], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('pet13', DiffPairs(x16_sigs[4], x16_sigs[5], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('per13', DiffPairs(x16_sigs[6], x16_sigs[7], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('pet14', DiffPairs(x16_sigs[8], x16_sigs[9], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('per14', DiffPairs(x16_sigs[10], x16_sigs[11], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('pet15', DiffPairs(x16_sigs[12], x16_sigs[13], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x16.append(
				Subsignal('per15', DiffPairs(x16_sigs[14], x16_sigs[15], dir = 'i', conn = conn, assert_width = 1))
			)
	...
	if io_x16 is not None:
		if all(x24_sigs):
			...
			io_x24.append(
				Subsignal('pet16', DiffPairs(x24_sigs[0], x24_sigs[1], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per16', DiffPairs(x24_sigs[2], x24_sigs[3], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet17', DiffPairs(x24_sigs[4], x24_sigs[5], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per17', DiffPairs(x24_sigs[6], x24_sigs[7], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet18', DiffPairs(x24_sigs[8], x24_sigs[9], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per18', DiffPairs(x24_sigs[10], x24_sigs[11], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet19', DiffPairs(x24_sigs[12], x24_sigs[13], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per19', DiffPairs(x24_sigs[14], x24_sigs[15], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet20', DiffPairs(x24_sigs[16], x24_sigs[17], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per20', DiffPairs(x24_sigs[18], x24_sigs[19], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet21', DiffPairs(x24_sigs[20], x24_sigs[21], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per21', DiffPairs(x24_sigs[22], x24_sigs[23], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet22', DiffPairs(x24_sigs[24], x24_sigs[25], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per22', DiffPairs(x24_sigs[26], x24_sigs[27], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('pet23', DiffPairs(x24_sigs[28], x24_sigs[29], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x24.append(
				Subsignal('per23', DiffPairs(x24_sigs[30], x24_sigs[31], dir = 'i', conn = conn, assert_width = 1))
			)
	...
	if io_x24 is not None:
		if all(x32_sigs):
			...
			io_x32.append(
				Subsignal('pet24', DiffPairs(x32_sigs[0], x32_sigs[1], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per24', DiffPairs(x32_sigs[2], x32_sigs[3], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet25', DiffPairs(x32_sigs[4], x32_sigs[5], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per25', DiffPairs(x32_sigs[6], x32_sigs[7], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet26', DiffPairs(x32_sigs[8], x32_sigs[9], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per26', DiffPairs(x32_sigs[10], x32_sigs[11], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet27', DiffPairs(x32_sigs[12], x32_sigs[13], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per27', DiffPairs(x32_sigs[14], x32_sigs[15], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet28', DiffPairs(x32_sigs[16], x32_sigs[17], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per28', DiffPairs(x32_sigs[18], x32_sigs[19], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet29', DiffPairs(x32_sigs[20], x32_sigs[21], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per29', DiffPairs(x32_sigs[22], x32_sigs[23], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet30', DiffPairs(x32_sigs[24], x32_sigs[25], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per30', DiffPairs(x32_sigs[26], x32_sigs[27], dir = 'i', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('pet31', DiffPairs(x32_sigs[28], x32_sigs[29], dir = 'o', conn = conn, assert_width = 1))
			)
			io_x32.append(
				Subsignal('per31', DiffPairs(x32_sigs[30], x32_sigs[31], dir = 'i', conn = conn, assert_width = 1))
			)
	...
	if not all((io_x4, io_x6, io_x8, io_x12, io_x16, io_x24, io_x32)) and pwrbrk_n is not None:
		warn(
