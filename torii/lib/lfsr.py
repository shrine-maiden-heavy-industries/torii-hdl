...
class LFSR(Elaboratable):
	...
	def __init__(
		...
		if poly_width > state_width + 1:
			warnings.warn(
				f'The polynomial {polynomial:X} is too wide for the LFSR ({poly_width} > {state_width}) and will be truncated.',
	...
	def _generate_galios(self, m: Module) -> None:
		''' Generate an "optimized" Galios-type LFSR '''
