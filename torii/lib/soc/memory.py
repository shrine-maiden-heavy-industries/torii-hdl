...
class MemoryMap:
	...
	def addr_width(self, addr_width):
		...
		if addr_width < self._addr_width:
			raise ValueError(f'Address width {addr_width!r} must not be less than its previous value {self._addr_width!r}, '
	...
	def _compute_addr_range(self, addr, size, step = 1, *, alignment, extend):
		...
		if addr > (1 << self.addr_width) or addr + size > (1 << self.addr_width):
			if extend:
				...
				raise ValueError(f'Address range {addr:#x}..{addr + size:#x} out of bounds for memory map spanning '
