	...
	class _Shadow:
		...
		def add(self, elem_range):
			...
			elem_range : :class:`range`
				Address range of a CSR :class:`Element`. It uses ``2 ** log2_int(elem_range.stop -
				elem_range.start, False)`` chunks of the shadow register. If this amount is greater than
				:attr:`~Multiplexer._Shadow.size`, it replaces the latter.
			'''
			assert isinstance(elem_range, range)
			self._ranges.add(elem_range)
			elem_size  = 2 ** log2_int(elem_range.stop - elem_range.start, False)
		...
		def decode_address(self, addr, elem_range):
			...
			:class:`int`
				...
				.. code-block::
					...
					+----+--+--+
							│  └─ 0
							└──── log2_int(elem_range.stop - elem_range.start, False)
