...
class ConstantStreamGenerator(Generic[T], Elaboratable):
	...
	def __init__(self,
		data: ConstData, data_width: int | None = None, max_length_width: int | None = None,
		endianness: Literal['big', 'little'] = 'little', stream_type: type[T] = StreamInterface,
		domain: str = 'sync'
	) -> None:
		self._domain           = domain
		self._data             = data
		...
		if max_length_width:
			...
			self.max_len = Const(self._data_width)
