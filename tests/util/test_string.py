...
class StringUtilTestCase(TestCase):
	...
	def test_ascii_escape(self):
		self.assertEqual(
			util_string.ascii_escape('abcdefg12346_'),
			'abcdefg12346_'
		)
		self.assertEqual(
		...
		self.assertEqual(
			util_string.ascii_escape('nya~'),
			'nya_7e_'
		)

	def test_tcl_escape(self):
		self.assertEqual(
			util_string.tcl_escape('uwu'),
			'{uwu}'
		)
		self.assertEqual(
			util_string.tcl_escape('"n"{y}a'),
			'{"n"\\{y\\}a}'
		)
		self.assertEqual(
	...
	def test_tcl_quote(self):
		self.assertEqual(
			util_string.tcl_quote('test'),
			'"test"'
		)
		self.assertEqual(
			util_string.tcl_quote('test"123'),
			'"test\\"123"'
		)
		self.assertEqual(
			util_string.tcl_quote('test[123'),
