...
class DecoderTestCase(ToriiTestSuiteCase):
	...
	def test_add_wrong_granularity_sparse(self):
		with self.assertRaisesRegex(
			...
			self.dut.add(
				Interface(
					addr_width = 15, data_width = 64, granularity = 16
				),
				sparse = True
			)
	...
	def test_add_wrong_optional_output(self):
		with self.assertRaisesRegex(
			...
			self.dut.add(
				Interface(
					addr_width = 15, data_width = 32, granularity = 16, features = {'err'}
				)
			)
...
class DecoderSimulationTestCase(ToriiTestSuiteCase):
	def test_simple(self):
		...
		sub_2 = Interface(
			addr_width = 14, data_width = 32, granularity = 8,
			features = {'err', 'rty', 'stall', 'lock', 'cti', 'bte'}
		)
...
class ArbiterSimulationTestCase(ToriiTestSuiteCase):
	def test_simple(self):
		...
		dut.add(intr_1)
		intr_2 = Interface(
			addr_width = 30, data_width = 32, granularity = 16,
			features = {'err', 'rty', 'stall', 'lock', 'cti', 'bte'}
		)
		dut.add(intr_2)
